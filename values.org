...
** Objection 1
One might ask, why can't it be the case that value properties are internal? That is to say, why can't it be sufficient that if one has the DE-sire^{1} that /x/. One might think that if one have the DE-sire that /x/, the DE-sire thereof is enough to say that there is motivation.
*** Response to 1
...
***** Argument 3a
1. If one has a DE-sire to correct a behavioral tendency, one has a tendency to not engage in said behavioral tendency.
...
+ 1. It should be clear how one is equivalent to the semantic thesis that having the propensity to behave in a way to bring /x/ about, one ought /x/.
** Objection 3
...
*** Response to 3
Even so, I can't see how [[Argument 3a]] doesn't apply. But if we assume for the sake of argument that 3a is mistaken with respect to synthetically reducible normative properties, I would point to Bilgrami's Moore-Frege-Moore Pincer Argument to quell these worries. That's too DE-tailed to be laid out here, but never the less, the argument can be given.


* Footnotes
1. DE-sires here are construed as all things considered DE-sires for the sake of simplicity. This is to avoid cases where one might not have the motivation to /x/ where there are in fact competing DE-sires to /~x/.
